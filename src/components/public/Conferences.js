...
function getConfList(confs, typef, filter) {
    ...
    switch (typef) {
        ...
        default: {
            typeret = confs.filter(conf => conf.type === typef)
        }
    }
...
function Conferences() {
    ...
    useEffect(() => {
        dispatch({ e: 'newconflist', data: getConfList(conferences, state.typefilter, state.filter) })
        if (state.filter === '') {
            searchRef.current.value = ''
        }
    }, [state.filter, state.typefilter, conferences])
